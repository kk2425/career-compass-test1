...
const getTagColor = (tag: AIRoadmap['careerTag']) => {
    switch (tag) {
        case "Entry-level friendly":
            return 'bg-green-500/20 text-green-300 border border-green-500/30';
        case "Mid-level (aspirational)":
            return 'bg-yellow-500/20 text-yellow-300 border border-yellow-500/30';
        case "Long-term vision role":
        ...
        >
            ...
            >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4">
                    ...
                    <span className={`mt-2 sm:mt-0 text-xs sm:text-sm font-semibold px-3 py-1 rounded-full whitespace-nowrap ${getTagColor(career.careerTag)}`}>
                        {career.careerTag}
                    </span>
                </div>
                ...
                <div className="mt-6 border-t border-slate-700 pt-6 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                     <div>
                        <h3 className="font-semibold text-white mb-2 text-lg">Learning Path</h3>
                        <p className="text-slate-400">{career.learningPath}</p>
                    </div>

                    {renderList(career.steppingStoneRoles, "Stepping Stone Roles")}
